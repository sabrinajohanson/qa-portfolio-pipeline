...
This repository contains no application code of its own. It's a single orchestrator workflow (`orchestrator.yml`) that, when manually triggered, runs the CI pipelines of the other 5 portfolio repositories in two phases, then builds and publishes a consolidated dashboard.
...
Each job in the Actions tab shows the live status of its corresponding repository's CI run, since the action polls the remote workflow until completion. Once finished, the updated dashboard is available at the link above.
